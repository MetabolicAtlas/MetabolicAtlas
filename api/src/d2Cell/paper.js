...
const formatDoi = (doi) => {
  ...
  if (doi.startsWith('10.')) {
    const firstDotIndex = doi.indexOf('.');
    const secondDotIndex = doi.indexOf('.', firstDotIndex + 1);
    if (secondDotIndex !== -1) {
      return doi.substring(0, secondDotIndex) + '/' + doi.substring(secondDotIndex + 1);
    }
  }
