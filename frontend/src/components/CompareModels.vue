        ...
        <div class="container is-fullhd">
            ...
            <div v-for="(c, id) in comparison" :key="id">
                <h4 class="title is-size-4">
                    {{ c.models.A.modelId }} vs. {{ c.models.B.modelId }}
                </h4>
...
export default {
    ...
    computed: {
        ...
        validModels() {
            return (
                this.selectedModels.length >= this.minModels &&
                this.selectedModels.length <= this.maxModels
            )
        },
    },
    watch: {
        ...
        async selectedModels(models) {
            ...
            if (
                JSON.stringify(query.models) !== JSON.stringify([this.$route.query.models].flat())
            ) {
    ...
    methods: {
        ...
        shouldDisable(model) {
            return (
                this.selectedModelIndex(model) === -1 &&
                this.selectedModels.length === this.maxModels
            )
        },
        async compare() {
            ...
            if (models) {
                const mappedModels = [models].flat().map((m) => {
                    ...
                    return this.modelList.find(
                        (x) => x.apiName === apiName && x.version === version
                    )
                })
