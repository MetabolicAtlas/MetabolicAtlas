                                ...
                                <template v-for="(w, i) in r.compartment_str.split(/⇔|⇒/)">
                                    ...
                                    <template v-for="(comp, j) in w.split(' + ')">
                                        ...
                                        <a
                                            :href="`/explore/${
                                                model.short_name
                                            }/gem-browser/compartment/${idfy(comp)}`"
                                            @click="handleRouterClick"
                                            >{{ comp }}</a
                                        >
                                    </template>
...
export default {
    ...
    computed: {
        ...
        sortedReactions() {
            ...
            return reactionsCopy
                .concat()
                .sort(compare(this.sortBy, this.sortPattern, this.sortOrder))
        },
    },
    watch: {
        sourceName() {
    ...
    methods: {
        async setup() {
            ...
            try {
                ...
                await this.$store.dispatch(
                    `reactions/getRelatedReactionsFor${this.type[0].toUpperCase()}${this.type.slice(
                        1
                    )}`,
                    payload
                )
        ...
        showCol(name) {
            if (
                (name === 'cp' && !this.showCP) ||
                (name === 'subsystem_str' && !this.showSubsystem)
            ) {
                return false
            }
            return true
