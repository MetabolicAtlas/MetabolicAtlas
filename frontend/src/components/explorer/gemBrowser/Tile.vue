<template>
    ...
    >
        <div
            class="tile is-child is-clickable box hoverable"
            :title="`Click to view ${data.name || data.id}`"
        >
            <p
                class="is-capitalized subtitle is-size-4-desktop is-size-5-tablet has-text-weight-light has-text-grey-light"
            >
                {{ label || type }}
            </p>
            <template v-if="type === 'metabolite'">
                ...
                <p v-if="data.reactionCount">
                    This compound is involved in <b>{{ data.reactionCount }}</b> reaction(s) across
                    the model.
                </p>
            </template>
            ...
            <template v-else-if="type === 'compartment'">
                ...
                <b>{{ data.geneCount }}</b> gene(s). <br /><br />
            ...
            <template v-else-if="type === 'subsystem'">
                ...
                <b>{{ data.metaboliteCount }}</b> metabolite(s) and
                <b>{{ data.geneCount }}</b> gene(s). This subsystem spans across
                <b>{{ data.compartmentCount }}</b> compartment(s).
            </template>
            <template v-else>
                <template v-if="data.name">
                    <b>{{ data.name }}</b> with {{ model.short_name }} ID <b>{{ data.id }}</b> is
                    associated with <b>{{ data.reactionCount }}</b> reaction(s) across
