<template>
    <div v-if="model" id="gem-search-wrapper">
        ...
        <div
            v-show="showResults && searchTermString.length > 1"
            id="searchResults"
            ref="searchResults"
        >
            <div
            ...
            <div v-show="!showLoader" v-if="searchResults.length !== 0" class="resList">
                <template v-for="type in resultsOrder">
                    ...
                    <hr
                        v-if="searchResults[type] && searchResults[type].length !== 0"
                        class="bhr p-0"
                    />
                </template>
            </div>
            ...
            <div v-show="!showLoader && noResult" class="has-text-centered notification m-0">
                ...
                <div v-if="notFoundSuggestions.length !== 0">
                    ...
                    <template v-for="v in notFoundSuggestions">
                        <a
                            :key="v"
                            class="suggestions has-text-link"
                            @click.prevent="searchDebounce(v)"
                            >{{ v }}</a
                        >&nbsp; </template
                    >?
                </div>
    ...
    methods: {
        ...
        blur() {
            setTimeout(() => {
                if (
                    document
                        .getElementById('gem-search-wrapper')
                        .contains(document.activeElement) === false
                ) {
                    this.showResults = false
                    this.handleClear()
                } else if (
                    document.getElementById('model-select').contains(document.activeElement) ===
                    false
                ) {
                    $('#search').focus()
                }
