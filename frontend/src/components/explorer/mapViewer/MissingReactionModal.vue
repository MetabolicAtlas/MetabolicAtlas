  ...
  <div v-if="showModal" class="modal is-active">
    ...
    >
      ...
      <p v-else class="pb-4">
        ...
        {{ currentMap.name }} maps. The 2D map of {{ currentMap.name }} is split into multiple maps
        due to its size. The number of missing reactions displayed is the missing reactions when the
        reactions in all {{ currentMap.name }} maps are combined. Some reactions are missing as the
        {{ currentMap.type }} is being updated much more often than the maps. Also, as the maps are
        manually curated, occasionally some reactions cannot be added. The number of reactions shown
        is {{ mapReactionList.length }}.
      </p>
