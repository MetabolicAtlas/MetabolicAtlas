        ...
        <div class="container is-fullhd">
            ...
            <div v-else>
                ...
                <p class="has-text-justified">
                    These models are integrated into the Metabolic Atlas database; they can be
                    explored via {{ messages.gemBrowserName }}, {{ messages.mapViewerName }} and
                ...
                <p class="has-text-justified">
                    While we do not provide support for these models, we are making them available
                    to download. For support, the authors should be contacted. They are listed in
                    the <i>References</i> section of each model. Click on a row to display more
                    information. To download multiple models at once use the
                                ...
                                <tbody>
                                    <tr v-for="field in model_fields" :key="field.name">
                                        ...
                                        >
                                            <td
                                                ...
                                                v-html="field.display"
                                            ></td>
                                            <td>
                                                {{
                                                    selectedModel[field.name] !== null
                                        ...
                                        <template
                                            v-else-if="
                                                typeof selectedModel[field.name] === 'boolean'
                                            "
                                        >
                                            <td
                                                class="td-key has-background-primary has-text-white-bis"
                                    ...
                                    <tr
                                        v-if="
                                            ...
                                            selectedModel.authors.length !== 0
                                        "
                                    >
                                        <td
                                            class="td-key has-background-primary has-text-white-bis"
                            ...
                            <template v-if="selectedModel.files">
                                ...
                                <template v-for="file in selectedModel.files">
                                    <a
                                        :key="file.path"
                                        class="button"
                                        :href="`/api/v2/repository/${file.path}`"
                                    >
                                        {{ file.format }} </a
                                    >&nbsp;
                                </template>
                                <div class="notification mt-4">
                                    ...
                                    <router-link
                                        :to="{ name: 'documentation', hash: '#FTP-access' }"
                                        >FTP server</router-link
                                    >.
                                </div>
                            </template>
    ...
    methods: {
        async getIntegratedModels() {
            try {
                ...
                if (urlId) {
                    const urlIntegrateModel = this.integratedModels.find(
                        (m) => m.short_name === urlId
                    )
