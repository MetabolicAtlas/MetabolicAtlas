                            ...
                            :class="[
                                {
                                    'is-active has-text-weight-bold':
                                        showTab(tab) && resultsCount[tab] !== 0,
                                },
                        ...
                        >
                            <div v-if="searchedTerm" class="has-text-centered notification">
                                ...
                                <b
                                    ><i>{{ searchedTerm }}</i></b
                                ><br />
                                If this is an alias or external identifier, it means it is not
                                present in any of the models.
                            </div>
                            <div class="content">
                        ...
                        <div v-show="showTab(header) && resultsCount[header] !== 0">
                            ...
                            >
                                ...
                                <template slot="table-row" slot-scope="props">
                                    ...
                                    <template
                                        v-else-if="['name', 'id'].includes(props.column.field)"
                                    >
                                        <router-link
                                            :to="{
                                                name: header,
                                    ...
                                    <template v-else-if="props.column.field === 'subsystem'">
                                        ...
                                        <template
                                            v-for="(sub, i) in props.formattedRow[
                                                props.column.field
                                            ]"
                                            v-else
                                        >
                                    ...
                                    <template
                                        v-else-if="
                                            ['compartment', 'compartments'].includes(
                                                props.column.field
                                            )
                                        "
                                    >
                                        ...
                                        <template
                                            v-else-if="
                                                ['gene', 'subsystem', 'reaction'].includes(header)
                                            "
                                        >
                                            <template
                                                v-for="(comp, i) in props.formattedRow[
                                                    props.column.field
                                                ]"
                                            >
                                    ...
                                    <template
                                        v-else-if="
                                            Array.isArray(props.formattedRow[props.column.field])
                                        "
                                    >
                                        {{ props.formattedRow[props.column.field].join('; ') }}
                                    </template>
                ...
                metabolite: [
                    ...
                    },
                    {
                        ...
                        filterOptions: {
                            enabled: true,
                            filterFn: (e, s) =>
                                e.filter((v) => v.name.toLowerCase().includes(s.toLowerCase()))
                                    .length !== 0,
                        },
                        sortable: true,
                    },
                ...
                compoList.forEach((el) => {
                    ...
                    if (componentType === 'metabolite') {
                        Object.keys(filterTypeDropdown[componentType]).forEach((field) => {
                            if (field === 'model') {
                                filterTypeDropdown[componentType][field][el[field].id] =
                                    el[field].name
                        ...
                        Object.keys(filterTypeDropdown[componentType]).forEach((field) => {
                            if (field === 'model') {
                                ...
                                el[field]
                                    .filter(
                                        (v) => !(v.id in filterTypeDropdown[componentType][field])
                                    )
                                    .forEach((v) => {
